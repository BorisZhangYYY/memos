...
describe("useMemoSave", () => {
  ...
  it("retries reminder linking against the saved memo without discarding the editor", async () => {
    mocks.memoSave
      .mockResolvedValueOnce({ hasChanges: true, memoName: "memos/new" })
      .mockResolvedValueOnce({ hasChanges: false, memoName: "memos/new" });
    const queryClient = new QueryClient();
    const wrapper = ({ children }: PropsWithChildren) => <QueryClientProvider client={queryClient}>{children}</QueryClientProvider>;
    const discardDraft = vi.fn();
    const onConfirm = vi.fn().mockRejectedValueOnce(new Error("reminder failed")).mockResolvedValueOnce(undefined);
    const { result } = renderHook(() => useMemoSave({ discardDraft, onConfirm }), { wrapper });
    await act(async () => result.current());
    expect(discardDraft).not.toHaveBeenCalled();
    expect(mocks.dispatch).not.toHaveBeenCalledWith({ type: "reset" });
    await act(async () => result.current());
    expect(mocks.memoSave.mock.calls[1][1]).toMatchObject({ memoName: "memos/new" });
    expect(discardDraft).toHaveBeenCalledOnce();
    expect(onConfirm).toHaveBeenNthCalledWith(2, "memos/new");
  });
});
