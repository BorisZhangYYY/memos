...
describe("useAutoSave (store-subscribed)", () => {
  ...
  it("persists content to the draft cache when content changes", () => {
    ...
    expect(saveSpy).toHaveBeenCalledWith(key, "hello world", [], expect.objectContaining({ moodLevel: 0 }));
  });
  ...
  it("persists uploaded attachments when editor metadata changes", () => {
    ...
    expect(saveSpy).toHaveBeenCalledWith(key, "", [image], expect.objectContaining({ moodLevel: 0 }));
  });
  ...
  it("persists metadata changes for an attachment with the same name", () => {
    ...
    expect(saveSpy).toHaveBeenCalledWith(key, "", [updated], expect.objectContaining({ moodLevel: 0 }));
  });
