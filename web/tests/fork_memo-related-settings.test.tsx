...
const mockInstance = {
  memoRelatedSetting: {
    reactions: ["👍"],
    moodEmojis: undefined as string[] | undefined,
...
describe("<MemoRelatedSettings> mood emojis", () => {
  ...
  it("shows the 7 default emojis when the setting has no moodEmojis", () => {
    mockInstance.memoRelatedSetting = { reactions: ["👍"], moodEmojis: undefined, contentLengthLimit: 8192 };
    renderSettings();
  ...
  it("restores the default emoji when a mood emoji is cleared before saving", () => {
    mockInstance.memoRelatedSetting = { reactions: ["👍"], contentLengthLimit: 8192, moodEmojis: DEFAULT_MOOD_EMOJIS };
    renderSettings();
  ...
  it("keeps custom non-empty mood emojis when saving", () => {
    const custom = ["💀", "😡", "😕", "😐", "🙂", "😊", "🤩"];
    mockInstance.memoRelatedSetting = { reactions: ["👍"], contentLengthLimit: 8192, moodEmojis: custom };
    renderSettings();

    // Dirty the form via the content length field so saving does not short-circuit on the unchanged state.
    fireEvent.change(screen.getByRole("spinbutton"), { target: { value: "16384" } });
    fireEvent.click(screen.getByRole("button", { name: "common.save" }));
