...
func TestSQLiteStaleRelationshipWriteCannotOutliveDeletedParent(t *testing.T) {
	...
	staleTx, err := ts.GetDriver().GetDB().Conn(ctx)
	require.NoError(t, err)
	// The fork deliberately defaults normal transactions to IMMEDIATE for ledger
	// consistency. Use an explicit deferred reader to exercise stale snapshots.
	defer staleTx.Close()
	_, err = staleTx.ExecContext(ctx, "BEGIN DEFERRED")
	require.NoError(t, err)
	staleTxOpen := true
	defer func() {
		if staleTxOpen {
			_, _ = staleTx.ExecContext(ctx, "ROLLBACK")
		}
	}()
	...
	_, err = staleTx.ExecContext(ctx, "ROLLBACK")
	require.NoError(t, err)
	staleTxOpen = false
