...
func TestFinanceLedgerMaintainsWalletBalances(t *testing.T) {
	...
	income, err := ts.CreateFinanceTransaction(ctx, &store.FinanceTransaction{
		UID: "finance-income", CreatorID: user.ID, OccurredTs: nowSec, Type: store.FinanceTransactionIncome,
	...
	transfer, err := ts.CreateFinanceTransaction(ctx, &store.FinanceTransaction{
		UID: "finance-transfer", CreatorID: user.ID, OccurredTs: nowSec, Type: store.FinanceTransactionTransfer,
	...
	adjustment, err := ts.CreateFinanceTransaction(ctx, &store.FinanceTransaction{
		UID: "finance-adjustment", CreatorID: user.ID, OccurredTs: nowSec, Type: store.FinanceTransactionAdjustment,
