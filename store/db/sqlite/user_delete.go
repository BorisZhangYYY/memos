...
func deleteUserFinanceTx(ctx context.Context, tx dbExecutor, userID int32) error {
...
func deleteMemosTx(ctx context.Context, tx dbExecutor, memoIDs []int32) error {
	for _, batch := range deleteUserBatches(memoIDs, deleteUserBatchSize) {
		clause, args := deleteUserInClause(1, batch)
		if _, err := tx.ExecContext(ctx, `UPDATE reminder SET memo_id = NULL WHERE memo_id IN `+clause, args...); err != nil {
			return err
		}
