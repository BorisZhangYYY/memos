...
func (s *Store) initializeInstanceAccessSetting(ctx context.Context) error {
	...
	legacyURL := s.profile.GetInstanceURL()
	if general.InstanceUrl != nil {
		legacyURL = general.GetInstanceUrl()
	}
	publicEnabled, err := s.PublicMemoEnabled(ctx)
	if err != nil {
		return err
	}
	if strings.TrimSpace(legacyURL) != "" && publicEnabled {
