...
func (s *Store) AllowsAnonymousAccess(ctx context.Context) (bool, error) {
	...
	if setting.AccessMode != storepb.InstanceAccessMode_INSTANCE_ACCESS_MODE_PUBLIC {
		return false, nil
	}
	return s.PublicMemoEnabled(ctx)
}
...
// PublicMemoEnabled reports whether PUBLIC memo visibility is enabled by the fork policy.
func (s *Store) PublicMemoEnabled(ctx context.Context) (bool, error) {
	setting, err := s.GetInstanceMemoRelatedSetting(ctx)
	if err != nil {
		return false, err
	}
	if len(setting.AllowedVisibilities) == 0 {
		return true, nil
	}
